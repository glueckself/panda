...
static void handle_query_supported(GdbCmdContext *gdb_ctx, void *user_ctx)
{
    ...
    g_string_printf(gdbserver_state.str_buf, "PacketSize=%x;ReverseContinue+;ReverseStep+", MAX_PACKET_LENGTH);
...
//TODO: panda: see if the functions can be reordered to avoid forward declarations
static void gdb_handle_panda_cmd(GdbCmdContext *gdb_ctx, void *user_ctx);

static GdbCmdParseEntry gdb_gen_query_set_common_table[] = {
    /* Order is important if has same prefix */
    {
        .handler = gdb_handle_panda_cmd,
        .cmd = "PandaCmd"
    },
    {
