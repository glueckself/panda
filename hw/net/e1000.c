            ...
            if (sz >= tp->tso_props.hdr_len
                ...
                if (rr_in_record()){
                    rr_record_net_transfer(RR_CALLSITE_E1000_PROCESS_TX_DESC_MEMMOVE_1,
                                           NET_TRANSFER_IOB_TO_IOB, (uint64_t)tp->data, (uint64_t)tp->header, tp->tso_props.hdr_len);
                }
            ...
            if (sz == msh) {
                ...
                if (rr_in_record()){
                    rr_record_net_transfer(RR_CALLSITE_E1000_PROCESS_TX_DESC_MEMMOVE_2,
                                           NET_TRANSFER_IOB_TO_IOB, (uint64_t)tp->header, (uint64_t)tp->data, tp->tso_props.hdr_len);
                }
